...
class MOTEvaluator:
    ...
    def evaluate_ocsort_sram(
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            # run tracking
            if outputs[0] is not None:
                online_targets = tracker.update_sram(outputs[0], info_imgs, self.img_size, self.args.mark_ration, model_sram, self.args.weight_1)
                online_tlwhs = []
                online_ids = []
                for t in online_targets:
                    tlwh = [t[0], t[1], t[2] - t[0], t[3] - t[1]]
    ...
    def evaluate_ocsort_stram(
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            # run tracking
            if outputs[0] is not None:
                online_targets = tracker.update_stram(outputs[0], info_imgs, self.img_size, self.args.mark_ration, model_stram, self.args.weight_s, self.args.weight_t, self.args.weight_st)
                online_tlwhs = []
                online_ids = []
                for t in online_targets:
                    tlwh = [t[0], t[1], t[2] - t[0], t[3] - t[1]]
    ...
    def evaluate_ocsort_tram(
        ...
        for cur_iter, (imgs, _, info_imgs, ids) in enumerate(
            ...
            # run tracking
            if outputs[0] is not None:
                online_targets = tracker.update_tram(outputs[0], info_imgs, self.img_size, model_tram_1, self.args.weight_1)
                online_tlwhs = []
                online_ids = []
                for t in online_targets:
                    tlwh = [t[0], t[1], t[2] - t[0], t[3] - t[1]]
