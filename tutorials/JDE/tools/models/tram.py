...
class TRAM(nn.Module):
    ...
    def forward(self, trajectories, humans):
        trajectories = self.linear_1(trajectories)
        humans = self.linear_2(humans)
        q_oe = k_oe = v_oe = torch.cat([trajectories, humans], 1)
        ...
        return tgt_oe[:, :trajectories_len, :], tgt_oe[:, trajectories_len:, :]

def build(args):
    return TRAM(args, args.d_model, args.hidden_dim)