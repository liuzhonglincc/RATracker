...
def eval_seq(opt, dataloader, data_type, result_filename,seq,save_dir=None, show_image=True, frame_rate=30):
    ...
    model_tram_1 = build_model(opt)
    model_tram_1.to(device)
    model_tram_1 = load_model(model_tram_1, "pretrained/model_t_1.pth")
    model_tram_1.eval()
    model_tram_2 = build_model(opt)
    model_tram_2.to(device)
    model_tram_2 = load_model(model_tram_2, "pretrained/model_t_2.pth")
    model_tram_2.eval()
    model_tram_3 = build_model(opt)
    model_tram_3.to(device)
    model_tram_3 = load_model(model_tram_3, "pretrained/model_t_3.pth")
    model_tram_3.eval()
    ...
    for path, img, img0,frame_id_start in dataloader:
        ...
        online_targets = tracker.update_tram(blob, img0, seq,save_dir, model_tram_1, model_tram_2, model_tram_3)
        online_tlwhs = []
        online_ids = []
...
if __name__ == '__main__':
    ...
    parser.add_argument('--val_mot17', default=True, help='val mot17')
    parser.add_argument('--val_mot20', default=False, help='val mot20')
    parser.add_argument('--test_mot20', default=False, help='test mot20')
    parser.add_argument('--val_hf', type=int, default=2, help='val_hf')

    parser.add_argument('--nms_thres', type=float, default=0.56, help='iou thresh for nms')
    parser.add_argument('--track_buffer', type=int, default=28, help='tracking buffer')
    ...
    parser.add_argument('--weights',type=str, default='../weights/mix_mot17_half_cstrack.pt', help='model.pt path(s)')
    parser.add_argument('--cfg', type=str, default='../experiments/model_set/CSTrack.yaml', help='model.yaml path')
    parser.add_argument('--conf_thres', type=float, default=0.57, help='object confidence threshold')
    ...
    parser.add_argument('--data_cfg', type=str,default='../dataset/MOT17/images/annotations/val_half.json',help='load data from cfg')
    parser.add_argument('--data_dir', type=str, default='//media/disk1/lzl/SOTS/CSTrack/dataset')
    ...
    parser.add_argument('--weight_1', type=float, default=0.7)
    parser.add_argument('--thresh_1', type=float, default=0.22)
    parser.add_argument('--weight_2', type=float, default=0.05)
    parser.add_argument('--thresh_2', type=float, default=0.8)
    parser.add_argument('--weight_3', type=float, default=0.5)
    parser.add_argument('--thresh_3', type=float, default=0.4)
