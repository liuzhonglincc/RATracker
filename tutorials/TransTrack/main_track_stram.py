...
from engine_track import evaluate, train_one_epoch, evaluate_stram
...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='mot_metrix/pred',
    ...
    parser.add_argument('--track_thresh', default=0.385, type=float)
    ...
    parser.add_argument('--weight_s', type=float, default=0.52)
    parser.add_argument('--weight_t', type=float, default=0.52)
    parser.add_argument('--weight_st', type=float, default=0.5)
    parser.add_argument('--thresh_1', type=float, default=0.82)
    return parser


def main(args, model_stram):
...
#             )
    ...
    if args.eval:
        ...
        tracker = Tracker(score_thresh=args.track_thresh, max_age=args.max_age)
        test_stats, coco_evaluator, res_tracks = evaluate_stram(model, criterion, postprocessors, data_loader_val,
                                                          base_ds, device, args.output_dir, tracker=tracker, 
                                                          phase='eval', det_val=args.det_val, fp16=args.fp16,
                                                          model_stram=model_stram, mark_ration =args.mark_ration,
                                                          max_age=args.max_age, weight_s=args.weight_s, weight_t=args.weight_t,
                                                          weight_st=args.weight_st, thresh_1=args.thresh_1)
        if args.output_dir:
...
#                 for _, img_info in dataset_val.coco.imgs.items():
        ...
        for seq_num in seqs:
            accs.append(eval_seq(seq_num, "TransTrack/mot_metrix/gt", "TransTrack/mot_metrix/pred"))
