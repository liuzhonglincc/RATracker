...
from engine_track import evaluate, train_one_epoch, evaluate_sram
...
def get_args_parser():
    ...
    parser.add_argument('--track_thresh', default=0.39, type=float)
    ...
    parser.add_argument('--thresh_1', type=float, default=1.1)
    parser.add_argument('--max_age', type=int, default=40)
    return parser


def main(args, model_sram):
...
#             )
    ...
    if args.eval:
        ...
        test_stats, coco_evaluator, res_tracks = evaluate_sram(model, criterion, postprocessors, data_loader_val,
                                                          base_ds, device, args.output_dir, tracker=tracker, 
                                                          phase='eval', det_val=args.det_val, fp16=args.fp16, 
                                                          model_sram=model_sram, weight_1=args.weight_1, thresh_1=args.thresh_1, mark_ration=args.mark_ration)
        if args.output_dir:
