...
class JDETracker(object):
    ...
    def update_tram(self, im_blob, img0, model_tram_1, model_tram_2, model_tram_3, 
                                            wight_1, thresh_1, wight_2, thresh_2, wight_3, thresh_3):
        self.frame_id += 1
        activated_starcks = []
        refind_stracks = []
        lost_stracks = []
        ...
        if len(strack_pool) > 0 and len(detections) > 0 :
            ...
            trajectories = torch.tensor(np.array(trajectories), dtype=torch.float32).unsqueeze(0).to(self.opt.device)
            humans = []
            for detection in detections:
                humans.append(detection.tlbr)
            humans = torch.tensor(np.array(humans), dtype=torch.float32).unsqueeze(0).to(self.opt.device)
            ...
            dists = wight_1*temporal_dist + (1-wight_1)*dists

        matches, u_track, u_detection = matching.linear_assignment(dists, thresh=thresh_1)
        ...
        if len(r_tracked_stracks) > 0 and len(detections) > 0 :
            ...
            humans_aligned = humans_aligned.squeeze(0) # detection
            temporal_dist = matching.embedding_distance_1(trajectories_aligned.tolist(), humans_aligned.tolist())
            dists = wight_2*temporal_dist + (1-wight_2)*dists
        matches, u_track, u_detection = matching.linear_assignment(dists, thresh=thresh_2)
        ...
        if len(unconfirmed) > 0 and len(detections) > 0 :
            ...
            humans_aligned = humans_aligned.squeeze(0) # detection
            temporal_dist = matching.embedding_distance_1(trajectories_aligned.tolist(), humans_aligned.tolist())
            dists = wight_3*temporal_dist + (1-wight_3)*dists

        matches, u_unconfirmed, u_detection = matching.linear_assignment(dists, thresh=thresh_3)
    ...
    def update_sram(self, im_blob, img0, model_sram):
        ...
        if len(detections) > 0 :
            ...
            humans = torch.tensor(np.array(humans), dtype=torch.float32).unsqueeze(0).to(self.opt.device)
            marks = torch.tensor(np.array(marks), dtype=torch.float32).unsqueeze(0).to(self.opt.device)
    ...
    def update_stram(self, im_blob, img0, model_stram):
        ...
        if len(detections) > 0 :
            ...
            if len(strack_pool) > 0:
                temporal_dist = matching.embedding_distance_1(trajectories_aligned.tolist(), humans_aligned.tolist())
                temporal_dist = self.opt.weight_t*temporal_dist + (1-self.opt.weight_t)*dists
            else:
                temporal_dist = dists
        if self.frame_id == 1:
            dists_fused = dists
            ...
            for detection in detections:
                human_aligned_feature.append(detection.fuse_reid.detach().cpu().numpy())
            spatial_dist = matching.embedding_distance_1(trajectory_aligned_feature, human_aligned_feature)
            spatial_dist = self.opt.weight_s*spatial_dist + (1-self.opt.weight_s)*dists
            dists_fused = self.opt.weight_st*temporal_dist + (1-self.opt.weight_st)*spatial_dist
            dists = dists_fused
