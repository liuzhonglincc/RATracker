import numpy as np
from .kalman_filter_1 import KalmanFilter
...
class ByteTrack(object):
    def __init__(self, args, frame_rate=30):
        ...
        if self.args.stram:
            self.model = build_model_st(self.args)
            if self.args.metric == "iou":
                self.args.match_thresh = 0.72
                self.args.weight_t = 0.2
                self.args.weight_s = 0.2
                self.args.weight_st = 0.16
            elif self.args.metric == "reid":
                self.args.match_thresh = 0.6
                self.args.weight_t = 0.3
                self.args.weight_s = 0.4
                self.args.weight_st = 0.5
            elif self.args.metric == "iou_reid":
                self.args.match_thresh = 0.7
                self.args.weight_t = 0.09
                self.args.weight_s = 0.09
                self.args.weight_st = 0.05
        else:
            self.model = None
        if self.model is not None:
            ...
            self.model.eval()
        if self.args.metric in ["reid", "iou_reid"]:
            self.encoder = FastReIDInterface(self.args.fast_reid_config, self.args.fast_reid_weights, self.device)
    ...
    def update(self, output_results, img):
        ...
        if self.args.metric in ["reid", "iou_reid"]:
            detections_reid = self.encoder.inference(img, bboxes)
            features_keep = detections_reid[remain_inds]
        
        if len(dets) > 0:
            '''Detections'''
            if self.args.metric in ["iou"]:
                detections = [STrack(STrack.tlbr_to_tlwh(tlbr), s) for
                            (tlbr, s) in zip(dets, scores_keep)]
            elif self.args.metric in ["reid", "iou_reid"]:
        ...
        if self.args.metric == "iou":
            ...
            dists_reid = matching.embedding_distance(strack_pool, detections)
            dists_iou = matching.iou_distance(strack_pool, detections)
            dists = 0.5*dists_reid + 0.5*dists_iou
        ...
        if self.args.metric in ["reid", "iou_reid"]:
            features_second = detections_reid[inds_second]
        ...
        if len(dets_second) > 0:
            ...
            if self.args.metric in ["iou"]:
                ...
                detections_second = [STrack(STrack.tlbr_to_tlwh(tlbr), s, feature_second) for
                                 (tlbr, s, feature_second) in zip(dets_second, scores_second, features_second)]
            else:
                detections_second = []
        else:
            detections_second = []
        ...
        if self.args.metric == "iou":
            ...
            dists_reid = matching.embedding_distance(r_tracked_stracks, detections_second)
